...
import com.wifishare.filesharing.datashare.smartshare.databinding.FragmentFileSelectionDocsBinding
...
class FileSelectionFragmentDocs : Fragment() , FileSelectionItem {

    private var binding : FragmentFileSelectionDocsBinding? = null
    ...
    override fun onCreateView(
        ...
        binding = FragmentFileSelectionDocsBinding.inflate(layoutInflater , container , false)
        ...
        BackgroundViewModel.docFile?.let { adapter?.setValues(requireContext() , it, "docs" , this) }
        ...
        GlobalScope.executeAsyncTask(
            onPreExecute = {
                ...
                BackgroundViewModel?.docFile?.forEach {
    ...
    override fun fileselectionitem(listofselection : ArrayList<FileInfo>) {
        if (listofselection.size == BackgroundViewModel.docFile?.size) {
            selectionclick = false
            val newColor = ContextCompat.getColor(requireContext(), R.color.mainthemecolor)
            binding?.selectImg?.setColorFilter(newColor, PorterDuff.Mode.SRC_IN)
            binding?.selectTxt?.text = "All Selected"

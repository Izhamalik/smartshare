...
import com.wifishare.filesharing.datashare.smartshare.databinding.FragmentFileSelectionMusicBinding
...
class FileSelectionFragmentMusic : Fragment(), FileSelectionItem {

    private var binding : FragmentFileSelectionMusicBinding? = null
    private var adapter : FilesSelectionOthersAdapter? = null
    ...
    override fun onCreateView(
        ...
        binding = FragmentFileSelectionMusicBinding.inflate(layoutInflater , container , false)

        binding?.recyclerView?.let{
            it.layoutManager = GridLayoutManager(requireContext() , 1)
            adapter= FilesSelectionOthersAdapter()
            it.adapter = adapter
        }
        BackgroundViewModel.audioFile?.let { adapter?.setValues(requireContext() , it, "music" , this) }
        ...
        GlobalScope.executeAsyncTask(
            onPreExecute = {
                ...
                BackgroundViewModel?.audioFile?.forEach {
    ...
    override fun fileselectionitem(listofselection : ArrayList<FileInfo>) {
        if (listofselection.size == BackgroundViewModel.audioFile?.size) {
            selectionclick = false
            val newColor = ContextCompat.getColor(requireContext(), R.color.mainthemecolor)
            binding?.selectImg?.setColorFilter(newColor, PorterDuff.Mode.SRC_IN)
            binding?.selectTxt?.text = "All Selected"

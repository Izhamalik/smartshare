...
class HistoryVideosAdapter : RecyclerView.Adapter<HistoryVideosAdapter.ViewHolder>() {
    ...
    lateinit var dataClickItem: DataClickItem
    ...
    fun setValues(
        ...
        dataClickItem: DataClickItem
    ) {
        this.itemList = itemlist
        this.context = context
        this.type = type
        this.fileSelectionItem = fileSelectionItem
        this.dataClickItem = dataClickItem
    }
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.itemView.setOnClickListener {
            if (!isLongClickEnabled) {
                if (itemList[position].fileType == 2){
                    itemList[position].filePath?.let { it1 -> dataClickItem.dataclickitem(it1 , "video") }

                }
                else{ itemList[position].filePath?.let { it1 -> dataClickItem.dataclickitem(it1 , "image") }
    ...
    fun longclickdisable(){
        isLongClickEnabled = false
    }
}

interface DataClickItem{
    fun dataclickitem(path : String , type : String)
}
